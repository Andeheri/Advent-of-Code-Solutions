...
while i < len_ID_ranges:
    ...
    if i != len_ID_ranges - 1:
        for j in range(i + 1, len_ID_ranges):
            ...
            if next_ingredient_ID_range[0] <= ingredient_ID_range[1]:
                # Merge ranges
                ingredient_ID_range = (ingredient_ID_range[0], max(ingredient_ID_range[1], next_ingredient_ID_range[1]))
            else:
                j -= 1
                break
        i = j + 1
...
for ingredient_ID_range in ingredient_ID_ranges:
    num_fresh_ingredient_IDs += ingredient_ID_range[1] - ingredient_ID_range[0] + 1

print(f"Number of fresh ingredient IDs: {num_fresh_ingredient_IDs}")